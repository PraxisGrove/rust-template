...
use {{crate_name}}_app::RecipientProvider;
...
mod tests {
    ...
    fn defaults_to_world_without_argument() {
        let provider = EnvRecipient::from_args(["{{project-name}}".to_owned()].into_iter());
    ...
    fn uses_first_user_argument_as_recipient() {
        let args = ["{{project-name}}".to_owned(), "Rust".to_owned()];
        let provider = EnvRecipient::from_args(args.into_iter());
